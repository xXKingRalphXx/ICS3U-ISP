...
public class ICS3U_FP_Final_Submission_Damilola{
	// global scanner (wrapper functions here are a bad idea)
	static Scanner s = new Scanner(System.in); 

	static int N; // square board side length
    ...
    */
	...
	public static void make_mv(int x0, int y0, int x1, int y1){
		...
		// king conversion checks
			...
			// has the player's regular piece reached the king's row? (the end of the board)
		if((board[x1][y1] == 1
		||  board[x1][y1] == 2)
	    && (isPlayerOne && x1 == N-1)
		|| (!isPlayerOne && x1 == 0)){
			...
			// no king has been crowned yet
			if(!firstKingWasSpawned){
				pretty_print("ALL HAIL THE KING");
				// a king was crowned — we'll never enter this if statement again
